...
public class Contest extends BaseEntity{
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        boolean allOk = qList.stream().allMatch(ques -> contestLevel.equals(ques.getLevel()));
        if (Boolean.FALSE.equals(allOk)) {
            throw new InvalidContestException("Questions cannot have other level type other than contest");
        }   
    }
    ...
    public void endContest(){
        if (ContestStatus.IN_PROGRESS.equals(this.contestStatus)) {
