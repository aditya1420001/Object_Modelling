...
public class WithdrawContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        final String contestId = tokens.get(1);
        final String userName = tokens.get(2);

        try {
            UserRegistrationDto withdrawContest = userService.withdrawContest(contestId, userName);
            System.out.println(withdrawContest);
        } catch (Exception e) {
