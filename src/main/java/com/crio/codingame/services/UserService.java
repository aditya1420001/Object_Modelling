...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        usersList.sort(Comparator.comparing(User::getScore));
        if (ScoreOrder.DESC.equals(scoreOrder)) Collections.reverse(usersList);
    ...
    Optional<User> user = userRepository.findByName(userName);
     ...
     if (contest.isEmpty()) throw new ContestNotFoundException();
     if (user.isEmpty()) throw new UserNotFoundException();

     if (!user.get().checkIfContestExists(contest.get())) throw new InvalidOperationException();
     if (contest.get().getCreator().equals(user.get())) throw new InvalidOperationException();


     ContestStatus contestStatus = contest.get().getContestStatus();
     
     if (ContestStatus.IN_PROGRESS.equals(contestStatus) 
        || ContestStatus.ENDED.equals(contestStatus)) {
